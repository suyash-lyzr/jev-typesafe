...
async function main() {
  ...
  for (const { preset, variant, reason } of run) {
    ...
    try {
      ...
      const missing = Object.keys(request.questions).filter((id) => !Object.hasOwn(body.answers, id))
      if (missing.length > 0) {
...
/**
 * Every recording in the folder, imported statically so the app can use them
 * as replays for variants the docs never published numbers for. The app only
 * uses one while its request still matches the variant exactly.
 */
function writeManifest(dir: string) {
  const files = readdirSync(dir).filter((f) => f.endsWith('.json')).sort()
  const lines = files.map((f, i) => `import r${i} from './${f}'`)
  const body = `// Generated by scripts/record-presets.ts. Do not edit by hand.
${lines.join('\n')}
...
export const OWN_RECORDINGS: OwnRecording[] = [${files.map((_, i) => `r${i}`).join(', ')}] as OwnRecording[]
`
